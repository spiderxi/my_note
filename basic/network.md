...
_讲一下网络的 OSI 七层模型和 TCP/IP 五层模型?_
...
_为什么有 MAC 地址还要有 IP 地址, 可以只是用 MAC 地址/IP 地址吗?_
...
_带宽(bandwidth)、吞吐量(throughput)、网速(speed)、延迟(latency)的区别?_
...
_为什么 ISP 宣称的 200Mbps 宽带实际网速才 3MiB/s?_
...
_Cookie 主要的组成部分有哪些？如何通过 Cookie 实现会话管理?_
...
```
存储密码时, 生成随机字符串作为salt, 将密码和salt组合再hash, DB中保存salt和hash值
```
