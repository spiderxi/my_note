# 1. 重构

## 1. 重构理论

***什么时候进行重构?***

```
* 发现代码重复时

* 新模块难以开发时

* 阅读旧代码时存在不便时
```

***什么是YAGNI?***

```
You aren't going to need it, 指在开发一个新功能时不必太考虑扩展性和泛用性, 当需要的时候重构代码即可
```

***重构的目的是什么?***

> 使人疲惫的不是远方的高山，而是鞋子里的一粒沙子。 --- 伏尔泰

## 2. Bad Smell

***说一说常见的代码异味以及解决方法?***

| Bad Smell                                | 解决方法                                     |
| ---------------------------------------- | -------------------------------------------- |
| **注释用来说明一段代码块的作用**   | 使用更好的命名替代注释                       |
| **多个代码块的代码结构相似**       | 提取函数                                     |
| 一个函数的参数列表过长                   | 参数封装成对象                               |
| **一个函数太长**                   | 按逻辑将一个函数拆分成多个函数               |
| **Magic String**                   | 使用枚举类/Static的字符串常量代替            |
| **多个基本变量同时出现, 同时消失** | 使用一个类封装变量                           |
| 重复出现的if/switch语句                  | 使用多态代替                                 |
| **对容器使用for循环语句**          | 使用Stream/管道替代                          |
| 一个容器类中只有字段                     | 将客户端对字段的操作代码封装到容器类中       |
| 频繁检查变量的取值是否合理               | JSR注解+校验框架                             |
| 使用复杂的计算表达式                     | 使用中间变量说明计算的含义                   |
| 直接使用object.a = b                     | 使用setter/getter方法                        |
| **死代码/被注释掉的代码**          | 直接删掉, 使用版本控制工具管理               |
| **if的条件是复杂的boolean表达式**  | 提取为isXxx()函数                            |
| **大量出现的else if**              | 使用if return替代                            |
| 返回null表示特殊含义                     | 返回特殊对象, 使用方法封装特殊对象的处理逻辑 |
| **滥用断言**                       | 断言只用于判断程序能够执行的必要条件         |
| 滥用setter                               | 如果一个对象在构造后变量只读, 则去掉setter   |
| 滥用继承                                 | 尽可能使用组合/接口替代继承                  |

# 2. 测试

## 1. 术语

***什么是测试驱动开发(TDD), 有什么好处?***

```
TDD指的是 先编写测试代码 -> 然后写具体的代码实现 ->  最后在每次编译后都执行测试代码 这样的过程

使用TDD的好处:
* 可以更早地发现bug
* 先编写测试代码有助于理解接口的功能
```

***测试用例如何设计?***

```
* 黑盒测试中常常使用等价类+边界分析生成测试用例, 测试用例一般为6+N个, 上/下边界附近的各三个用例, N个正常用例

* 白盒测试注意分支覆盖和语句覆盖
```

***什么是断言, 有什么注意事项?***

```
断言(assert)用于检查程序要正常运行必须要满足的条件

注意: 生产环境中不允许使用断言
```

## 2. Junit

***为什么Junit的单元测试类中没有Main函数却可以正常运行?***

```
* maven进程在Test phase阶段的插件中调用了Junit中的测试API:

               JUnitCore.runClasses(TestClass.class)
```

# 3. 设计模式

***说一下面向对象的设计原则?***

```
* 开闭原则
* 合成复用原则: 组合优于继承
* 单一职责原则: 接口职责/类的职责要单一
* 依赖倒置原则: 依赖于接口编程
* Demeter原则: 一个类只需要知道它需要知道的最少信息
* liskov替换原则: 子类在任何地方都能替代父类
```

***lazy-loading的单例模式实现方式有哪些?***

```
* DCL
* static 代码块
* 枚举类
```

***代理模式和装饰器模式的区别?***

```
代理模式一般只会代理一层, 装饰器模式一般封装多层
```

# 4. 代码规范

## 1. 通用代码规范

***如何命名?***

| 类型                   | 命名方式                        |
| ---------------------- | ------------------------------- |
| 类                     | XxxYyy                          |
| 变量/方法              | xxxYyy                          |
| 类中的final static变量 | XYZ_IJK                         |
| 抽象类                 | AbstractXxx                     |
| 枚举类                 | XxxEnum                         |
| 单一接口实现类         | XxxxImpl                        |
| 包名/路径名            | xxx.yyy                         |
| 特殊作用的数据容器类   | XxxDTO  XxxVo XxxDAO XxxMapper |

***良好的代码风格需要注意的事项有哪些?***

| 方面       | 注意事项                                                                                                                                         |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| 代码格式   | 缩进使用4个空格;<br />多目运算符两边使用空格;<br />分支语句使用{};<br />左结合符(如;)的右边有空格;<br />字面常量一律使用大写后缀(L,D,F);         |
| 代码统一   | 编码一律使用UTF8;<br />换行符一律使用LF;                                                                                                         |
| 代码正确性 | 使用 `常量.equals(obj)`的方式防止NPE;<br />尽可能使用包装类而不是基本类型;<br />Boolean类型不要使用is作为前缀;<br />重写equals必须重写hashcode |
| 代码可读性 | 抽象的方法必须添加javadoc注释;                                                                                                                   |

## 2. javadoc注释

***javadoc注释中常用的标签有哪些?***

```
@date 2023/8/19
@author 
@since <version>
----------
@param	参数说明
@return 返回值说明
@deprecated 过期说明
-----------
* @see 包.类#成员	-- 相关的内容
{@code 代码}	-- 代码块
{@link 包.类#成员}	--跳转
{@value 包.类#static成员}   --显示static变量的值
```

***pakage-info.java的作用是什么?***

```
提供包级别的javadoc文档
```

## 3. lombok

***lombok中常用的注解有哪些?***

| 注解                          | 作用/使用示例                 |
| ----------------------------- | ----------------------------- |
| **@Getter**             |                               |
| **@Setter**             | `@Setter(AccessLevel.NONE)` |
| @ToString                     |                               |
| @EqualsAndHashCode            |                               |
| **@XxxArgsConstructor** |                               |
| **@Data**               |                               |
| **@Builder**            | 支持链式调用初始化            |
| **@Slf4j**              | 支持log                       |
| **@SneakyThrow**        |                               |

***lombok注解如何生效?***
