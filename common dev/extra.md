...
_系统代理和应用程序代理的区别?_
```
🌟系统代理: 操作系统级别的网络代理配置, 在使用操作系统提供的网络API时(如WinINet)默认会走系统代理
🌟应用程序代理: 指应用程序自定义的网络代理, 应用程序调用底层socket API可绕过系统代理使用自定义的网络代理

🌙VPN的原理就是修改系统代理为VPN应用绑定的端口localhost:port, 并转发网络包
🌙浏览器默认支持系统代理, 其他应用程序不一定支持系统代理, 但可以环境变量获取系统代理配置
```
