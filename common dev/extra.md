...
```
🌟 unix: 使用包管理器(如apt yum, 包管理器在安装时可同步修改PATH变量)
...
🌙不同系统的API定义可能不一样， 所以应用程序有win/mac/linux等版本（基于Java等跨平台的应用程序除外）
...
*时间戳和本地时间的区别是什么?*
```
🌟 全球任何地方任何一刻, 同一个时间标准下的时间戳的值都相等

🌟 同一个时间标准同一个时间戳下, 不同地区的本地时间字符串表示不同(如同一时刻, 上海UTC时间显示8:00AM, 纽约UTC时间显示7:00PM)
```

*TAI GMT UT1 UTC CST 时间标准的区别是什么?*
```
🌟 TAI=原子钟时间, 基于铯原子钟, 相邻两个时间戳之间对应的物理时间间隔是均匀的
🌟 UTC=协调世界时
🌟 GMT=格林尼治时间, 等价于UTC+0
🌟 CST=中国标准时间, 等价于UTC+8

🌙 GMT历史上和UT1一样基于天文观测, 但现在使用时一般表示UTC+0
🌙 unix like系统中使用的是UTC时间戳
```
...
🌙 操作系统每隔几分钟就会自动从时间服务器(ip可配置或使用预设服务器)同步时间(中国的时间服务器为国家授时中心)
🌙 NTP除了同步时间戳外, 还会同步闰秒信息
```

*操作系统是怎么处理闰秒的?*
```
Unix时间戳的数值序列上不体现闰秒。操作系统通过NTP等服务是知道闰秒的，只是选择了不直接在时间戳上“插入”那一秒，而是通过涂抹技术平滑处理(24h平滑期)
...
| --------------------- | ------------------------- |
| 执行一个指令          | 1 ns                      |
| L1-L3 cache 访问     | L1约1 ns, L3约50 ns |
| Memory 访问           | 100 ns                    |
| SSD 随机访问/顺序访问 | 0.01~0.1 ms                    |
| HDD 顺序访问          | 2ms                       |
| HDD 随机访问          | 20ms                    |
| 北京到上海网络延迟    | 30ms                      |
...
_为什么尽可能使用内存存储和顺序访问_
```
🌟 即使不考虑L1~L3缓存, 访问磁盘的时间也是访问内存的时间的1万倍
🌟 随机访问的时间是顺序访问的10倍

🌙 多数数据库都喜欢立刻保存日志而不是立刻保存数据, 就是因为日志是顺序写入, 而数据是随机写入
```
