...
## 基础神经网络
_在数字识别的三层神经网络中特征平面对应哪些特征?_
```
🌟 浅层神经元对应某位置是否为横线/竖线/左弧线（如左下角是否为横线）
🌟 深层神经元对应某位置是否有圆圈（可通过上层神经元判断能否组合成圆圈）
```
...
```
🌟 将输入信号X正则化为0~1之间的值
🌟 重复计算隐藏层神经元值A = 隐藏层激活函数(W.X + B)
🌟 计算输出层Y = 输出层激活函数(A)
```

_为什么增加神经网络深度比增加宽度效果更好？_
```
🌟 近似上， 计算次数随宽度平方增加， 随深度线性增加
...
🌙 正是有了BP算法， 神经网络才得以发展， BP算法可能灵感也来源与人类的神经元反馈过程
```
