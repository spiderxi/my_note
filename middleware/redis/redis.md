...
***Redis Cluster的作用和原理?***
```
原理: 使用一致性哈希算法, 将key映射为slot, 请求时直接请求slot对应的主从节点(一主多从), 写请求走主节点, 读请求走从节点
作用: 便于横向扩展数据容量
```
...
***什么是缓存穿透问题?***
```
大量请求访问了DB中不存在的数据(此时缓存层没有起作用, 每次都会请求DB)
...
🌙将value设置为IP_ThreadId目的是为了防止线程处理超时, 解锁了其他线程的锁
